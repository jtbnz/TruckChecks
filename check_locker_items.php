...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['check_items'])) {
    ...
    // Get all items for this locker and insert check items in bulk
    ...
    // Prepare bulk insert for check_items
    $check_item_values = [];
    $check_item_params = [];
    foreach ($items as $item) {
        $is_present = in_array($item['id'], $checked_items) ? 1 : 0;
        $check_item_values[] = '(?, ?, ?)';
        $check_item_params[] = $check_id;
        $check_item_params[] = $item['id'];
        $check_item_params[] = $is_present;
    }
    
    if (!empty($check_item_values)) {
        $check_item_sql = 'INSERT INTO check_items (check_id, item_id, is_present) VALUES ' . implode(', ', $check_item_values);
        $check_item_query = $db->prepare($check_item_sql);
        $check_item_query->execute($check_item_params);
    }
}

// Fetch trucks - filter by station if available (optimized with single query)
if ($current_station) {
...
if ($selected_truck_id) {
    ...
    if ($selected_locker_id) {
        // Optimized: Get all locker data in fewer queries
        ...
        // Get items for the selected locker
        if ($RANDORDER) {
        ...
        // Get last check info and last 5 checks in one optimized query
        ...
        $check_results = $last_check_query->fetchAll(PDO::FETCH_ASSOC);
        
        if (!empty($check_results)) {
            $last_check = $check_results[0]; // Most recent check
            $last_five_checks = $check_results; // All 5 checks
        ...
        // Get last notes from most recent check
        $last_notes_query = $db->prepare("
            SELECT cn.note 
            FROM check_notes cn
            ...
            LIMIT 1
        ");
        $last_notes_query->execute(['locker_id' => $selected_locker_id]);
        $last_notes = $last_notes_query->fetchColumn() ?: '';
    }
}
?>
...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (lastCheckedBy && checkedByInput) {
            ...
            if (form) {
                form.addEventListener('submit', function() {
                    const checkedBy = checkedByInput ? checkedByInput.value : '';
                    if (checkedBy) {
